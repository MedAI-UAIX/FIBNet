...
class ModelTrainer(object):
    ...
    def train(data_loader, model, loss_f, optimizer, epoch_id, device, max_epoch):
        ...
        for i, data in enumerate(data_loader):
            ...
            pred_prob = prob[:,1].cpu().detach().numpy()
            y_prob = np.append(y_prob, pred_prob)
            y_truth = np.append(y_truth, labels.cpu().numpy())
            # print(2)
        ...
        fpr_train, tpr_train, thresholds_train = roc_curve(y_truth, y_prob)  #测试集ROC
        auc_train = auc(fpr_train, tpr_train)
        return np.mean(loss_sigma), acc_avg, conf_mat, auc_train
    ...
    def valid(data_loader, model, loss_f, device):
        ...
        acc_avg = conf_mat.trace() / conf_mat.sum()
        fpr_test, tpr_test, thresholds_test = roc_curve(y_truth, y_prob)  #测试集ROC
        auc_valid = auc(fpr_test, tpr_test)
      
        return np.mean(loss_sigma), acc_avg, conf_mat, auc_valid
    ...
    if verbose:
        for i in range(cls_num):
            ...
            with open(os.path.join(out_dir, 'log.log'), 'a') as f:
                f.write('class:{:<10}, total num:{:<6}, correct num:{:<5}  Recall: {:.2%} Precision: {:.2%}\n'.format(
                        classes[i], np.sum(confusion_mat[i, :]), confusion_mat[i, i],
                        confusion_mat[i, i] / (.1 + np.sum(confusion_mat[i, :])),
                        confusion_mat[i, i] / (.1 + np.sum(confusion_mat[:, i]))))
